    ...
    case "ftc-experimental-warning":
      {
        ...
        dialog
          .showMessageBox(window, {
            ...
            detail:
              "Support for FTC fields in PitchScope is an experimental feature, and may not function properly in all cases. Please report any problems via the GitHub issues page.",
            buttons: ["OK"],
            checkboxLabel: "Don't Show Again",
            icon: WINDOW_ICON
          })
      ...
      if (message.data.incompleteWarning) {
        dialog
          .showMessageBox(window, {
            ...
            detail:
              'Some fields will not be available in the exported data. To save all fields from the server, the "Logging" live mode must be selected. Check the PitchScope documentation for details.',
          ...
          click() {
            dialog
              .showSaveDialog({
                ...
                defaultPath: "PitchScope " + new Date().toLocaleDateString().replaceAll("/", "-") + ".json",
                properties: ["createDirectory", "showOverwriteConfirmation", "dontAddToRecent"],
                ...
                if (files.filePaths.length > 0) {
                  ...
                  if (
                    ...
                    dialog.showMessageBox({
                      ...
                      detail: oldLayout
                        ? "The selected layout file uses an older format which is not compatible with the current version of PitchScope."
                  ...
                  if (app.isPackaged && data.version !== APP_VERSION) {
                    let result = dialog.showMessageBoxSync({
                      ...
                      detail:
                        "The layout file was generated by a different version of PitchScope. Compatability is not guaranteed.",
                      buttons: ["Continue", "Cancel"],
                      icon: WINDOW_ICON
                    });
                    if (result !== 0) return;
...
/** Creates the "About PitchScope" window. */
      ...
      if (isBetaExpired()) {
          ...
          .showMessageBox(window, {
            ...
            detail:
              ...
              (DISTRIBUTION === Distribution.WPILib
                ...
                : "Please download the latest stable release of PitchScope from GitHub."),
            buttons: ["Quit", "Ignore"],
            defaultId: 0
          })
          .then((result) => {
        ...
        dialog
          .showMessageBox(window, {
            ...
            detail: `Please take 30 seconds to share your experience using the PitchScope ${
              isAlpha() ? "alpha" : "beta"
...
if (process.platform === "linux") {
  if (DISTRIBUTION === Distribution.WPILib) {
    app.setName("PitchScope (WPILib)");
  } else {
    app.setName("PitchScope");
  }
}
